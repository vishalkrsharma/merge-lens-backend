...
export class FindingsController {
  ...
  getHotspots(
    ...
    return this.findingsService.getHotspots(
      user.id,
  ...
  @Get()
  @ApiOperation({
    summary: 'List findings across all reviews with optional filters',
  })
  @ApiQuery({
    name: 'agent',
    required: false,
    enum: ['bug', 'security', 'performance', 'style', 'all'],
  })
  @ApiQuery({
    name: 'severity',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description:
      'Paginated findings. Each item includes review: { id, owner, repo }.',
    schema: {
      properties: {
